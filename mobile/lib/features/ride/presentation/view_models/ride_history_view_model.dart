...
class RideHistoryUiState {
  const RideHistoryUiState({
    this.phase = RideHistoryPhase.initialLoading,
    this.rides = const [],
    this.nextCursor,
    this.status = 'all',
    this.serviceType,
  ...
  bool get canLoadMore =>
      hasMore && !isLoadingMore && !isRefreshing && phase != RideHistoryPhase.initialLoading;

  RideHistoryUiState copyWith({
    RideHistoryPhase? phase,
    List<Ride>? rides,
    String? nextCursor,
    String? status,
    String? serviceType,
...
/// Passenger ride history — server list only; ownership decided by backend.
class RideHistoryViewModel extends AutoDisposeNotifier<RideHistoryUiState> {
  ...
  Future<void> loadInitial() async {
    ...
    state = state.copyWith(
      phase: RideHistoryPhase.initialLoading,
      rides: const [],
  ...
  Future<void> refresh() async {
    if (_refreshInFlight || state.phase == RideHistoryPhase.initialLoading) {
      return;
    }
  ...
  Future<void> setStatus(String status) async {
    ...
    if (normalized == state.status &&
        state.phase != RideHistoryPhase.fatalError) {
      return;
    }
    ...
    try {
      ...
      state = state.copyWith(
        phase: merged.isEmpty
            ? RideHistoryPhase.empty
            : RideHistoryPhase.ready,
        rides: merged,
      ...
      if (append && state.rides.isNotEmpty) {
        ...
        state = state.copyWith(
          ...
          phase: RideHistoryPhase.ready,
        );
      } else {
        state = state.copyWith(
          phase: RideHistoryPhase.fatalError,
          isRefreshing: false,
          isLoadingMore: false,
          errorMessage: failure.userMessage,
          rides: const [],
...
final rideHistoryViewModelProvider =
    NotifierProvider.autoDispose<RideHistoryViewModel, RideHistoryUiState>(
  RideHistoryViewModel.new,
);
